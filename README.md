...
- **Owner role, Activity Log, and admin-action notifications** (`smartgrocer/audit.py`, `notifications.py`, `staff.py`, Activity Log screen) - a security layer built from the shop owner's own request: an Admin should be able to manage staff day-to-day, but the Owner should be able to see exactly who did what, and hear about it as it happens rather than only on request. Concretely: (1) a third role, **Owner**, sits above Admin (Owner can do everything Admin can, plus view the Activity Log) - only an existing Owner can create another Owner account, *except* that the very first Owner can be created by any Admin, so a fresh install isn't locked out of ever having one; (2) every sensitive staff action - adding staff, resetting a PIN, activating/deactivating someone - is written to an `audit_log` table recording who did it, their role, who it was done to, and when, with the names/roles snapshotted at that moment so the log still reads correctly even after someone is later renamed or deactivated; (3) the same actions also trigger a best-effort notification naming which Admin/Owner did what (e.g. "Admin Kasun changed the PIN for Nimal") - **email** sends immediately once you fill in a Gmail address + "app password" in the new Activity Log screen's Notification Settings, while **SMS/WhatsApp** need your own paid Twilio account (there's no free or keyless way to send either from any provider) - the Twilio sending code is complete and real, it's simply never called until real credentials are filled in, and hasn't been tested against a live account since none exists in this environment; a notification failing to send (bad password, no internet) never blocks the action itself. A cloud-based system for checking shop details remotely is a natural next step here but deliberately not built yet - noted for later, not part of this round.
